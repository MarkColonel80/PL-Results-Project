...
export default function Team(){
 ...
 return <>
  ...
  <div className="grid"><section className="card"><h2>Results</h2>{matches.map(m=><a href={`/matches/${encodeURIComponent(m.match_id)}`} key={m.match_id}><div className="team-result-row"><span className={`result-${result(m)}`}>{result(m)}</span><span className="muted">GW {m.gameweek}</span><span>{m.home_team} <b>{m.home_score}–{m.away_score}</b> {m.away_team}</span></div></a>)}</section><section className="stack"><div className="card"><h2>Top scorers</h2><div className="tablewrap"><table><thead><tr><th>Player</th><th>G</th><th>A</th></tr></thead><tbody>{topScorers.map(x=><tr key={x.player_name}><td><a className="textlink" href={`/players/${encodeURIComponent(x.player_name)}?season=${encodeURIComponent(season)}`}>{x.player_name}</a></td><td><b>{x.goals}</b></td><td>{x.assists}</td></tr>)}</tbody></table></div></div><div className="card"><h2>Top creators</h2><div className="tablewrap"><table><thead><tr><th>Player</th><th>A</th><th>G</th></tr></thead><tbody>{topCreators.map(x=><tr key={x.player_name}><td><a className="textlink" href={`/players/${encodeURIComponent(x.player_name)}?season=${encodeURIComponent(season)}`}>{x.player_name}</a></td><td><b>{x.assists}</b></td><td>{x.goals}</td></tr>)}</tbody></table></div></div></section></div>
 </>;
}