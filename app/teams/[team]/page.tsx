...
export default function Team(){
 ...
 useEffect(()=>{(async()=>{const[{data:s},{data:m},{data:pl}]=await Promise.all([
  ...
  supabase.from("player_season_stats").select("*").eq("season",season).eq("team_name",team)
 ...
 const topScorers=useMemo(()=>[...players].filter(p=>p.goals!=null).sort((a,b)=>(b.goals||0)-(a.goals||0)||(b.assists||0)-(a.assists||0)).slice(0,8),[players]);
 const topCreators=useMemo(()=>[...players].filter(p=>p.assists!=null).sort((a,b)=>(b.assists||0)-(a.assists||0)||(b.goals||0)-(a.goals||0)).slice(0,8),[players]);
 const playerLink=(x:any)=>`/players/${encodeURIComponent(x.player_id)}?season=${encodeURIComponent(season)}`;
 return <>
  ...
  <div className="grid"><section className="card"><h2>Results</h2>{matches.map(m=><a href={`/matches/${encodeURIComponent(m.match_id)}`} key={m.match_id}><div className="team-result-row"><span className={`result-${result(m)}`}>{result(m)}</span><span className="muted">GW {m.gameweek}</span><span>{m.home_team} <b>{m.home_score}–{m.away_score}</b> {m.away_team}</span></div></a>)}</section><section className="stack"><div className="card"><h2>Top scorers</h2>{topScorers.length?<div className="tablewrap"><table><thead><tr><th>Player</th><th>G</th><th>A</th></tr></thead><tbody>{topScorers.map(x=><tr key={x.player_id}><td><a className="textlink" href={playerLink(x)}>{x.player_name}</a></td><td><b>{x.goals}</b></td><td>{x.assists}</td></tr>)}</tbody></table></div>:<p className="muted">ID-linked scoring stats not loaded yet.</p>}</div><div className="card"><h2>Top creators</h2>{topCreators.length?<div className="tablewrap"><table><thead><tr><th>Player</th><th>A</th><th>G</th></tr></thead><tbody>{topCreators.map(x=><tr key={x.player_id}><td><a className="textlink" href={playerLink(x)}>{x.player_name}</a></td><td><b>{x.assists}</b></td><td>{x.goals}</td></tr>)}</tbody></table></div>:<p className="muted">ID-linked assist stats not loaded yet.</p>}</div></section></div>
 </>;
}