...
export default function Leaders(){
 ...
 useEffect(()=>{let cancelled=false;(async()=>{setLoading(true);const[{data:leaders},{data:summary}]=await Promise.all([
   supabase.rpc("get_player_season_leaders",{p_season:season}),
 ...
 return <><div className="hero"><div><h1>Player leaders</h1><div className="muted">Search players and rank by football or FPL output. Player goal totals exclude own goals.</div>{goalSummary&&<div className="recordline" style={{marginTop:8}}><b>{matchGoals} league goals</b><span>= {playerGoals} player goals{ownGoals?` + ${ownGoals} own goal${ownGoals===1?"":"s"}`:""}</span></div>}</div><SeasonSelect season={season} onChange={setSeason} includeHistory/></div><div className="filterbar"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search player or team…"/><select value={sort} onChange={e=>setSort(e.target.value)}>{Object.entries(sorts).map(([k,v]:any)=><option value={k} key={k}>Sort: {v[1]}</option>)}</select></div>{loading?<div className="card"><span className="muted">Loading leaders…</span></div>:<div className="tablewrap"><table><thead><tr><th>#</th><th>Player</th><th>Team</th><th>Apps</th><th>Min</th><th>Goals</th><th>Assists</th><th>G+A</th><th>FPL</th></tr></thead><tbody>{visible.map((r,i)=><tr key={`${r.player_code}-${r.team_name}`}><td>{i+1}</td><td><a href={`/players/${encodeURIComponent(r.player_code)}?season=${encodeURIComponent(season)}`}><b>{r.player_name}</b></a></td><td>{r.team_name?<a href={`/teams/${encodeURIComponent(r.team_name)}?season=${encodeURIComponent(season)}`}>{r.team_name}</a>:"—"}</td><td>{r.appearances}</td><td>{Math.round(Number(r.minutes||0))}</td><td>{show(r.goals)}</td><td>{show(r.assists)}</td><td><b>{show(r.goal_contributions)}</b></td><td>{show(r.fpl_points)}</td></tr>)}</tbody></table></div>}</>;
}
