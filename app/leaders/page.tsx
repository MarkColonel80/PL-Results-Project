...
export default function Leaders(){
 ...
 return <><div className="hero"><div><h1>Player leaders</h1><div className="muted">Search players and rank by the stat that matters.</div></div><SeasonSelect season={season} onChange={setSeason}/></div><div className="filterbar"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search player or team…"/><select value={sort} onChange={e=>setSort(e.target.value)}>{Object.entries(sorts).map(([k,v]:any)=><option value={k} key={k}>Sort: {v[1]}</option>)}</select></div><div className="tablewrap"><table><thead><tr><th>#</th><th>Player</th><th>Team</th><th>Apps</th><th>Starts</th><th>Goals</th><th>Assists</th><th>G+A</th></tr></thead><tbody>{visible.map((r,i)=><tr key={`${r.player_name}-${r.team_name}`}><td>{i+1}</td><td><a href={`/players/${encodeURIComponent(r.player_name)}?season=${encodeURIComponent(season)}`}><b>{r.player_name}</b></a></td><td><a href={`/teams/${encodeURIComponent(r.team_name)}?season=${encodeURIComponent(season)}`}>{r.team_name}</a></td><td>{r.appearances}</td><td>{r.starts}</td><td>{r.goals}</td><td>{r.assists}</td><td><b>{r.goal_contributions}</b></td></tr>)}</tbody></table></div></>;
}