...
export default function WeekendVenueReview(){
 ...
 return <>
  ...
  <section className="card"><b>Adjusted venue xG rule</b><div className="muted" style={{marginTop:6}}>For each of the eight venue matches, actual goals scored and conceded are first capped to within ±1.0 goal of that match&apos;s xG. Those capped actual-goal averages are then blended 50/50 with raw venue xG: adjusted xGF8 = (xGF8 + capped actual GF8) ÷ 2, with the same rule for xGA. This stops freak finishing or goalkeeping scorelines dominating an eight-match sample while still allowing actual goals to add information beyond xG. All prices below are decimal odds; bookmaker probabilities are no-vig estimates from the Oddschecker 1X2 prices.</div></section>
  <div style={{display:"grid",gap:14}}>{cards.map(({f,s,m,calc})=><section className="card" key={f.fixture_id}>
   ...
   {!s?<p>No snapshot.</p>:<>
    <div style={{overflowX:"auto",marginTop:10}}><table><thead><tr><th>Venue 8</th><th>{f.home_team}</th><th>{f.away_team}</th></tr></thead><tbody>
     ...
     <tr><td>Actual GF</td><td>{fmt(s.home_vgf8)}</td><td>{fmt(s.away_vgf8)}</td></tr><tr><td>Capped actual GF (±1 xG)</td><td>{fmt(s.home_vgf8_capped)}</td><td>{fmt(s.away_vgf8_capped)}</td></tr><tr><td>Raw xGF</td><td>{fmt(s.home_vxgf8)}</td><td>{fmt(s.away_vxgf8)}</td></tr><tr><td><b>Adjusted xGF</b></td><td><b>{fmt(calc?.ahf)}</b></td><td><b>{fmt(calc?.aaf)}</b></td></tr>
     <tr><td>Actual GA</td><td>{fmt(s.home_vga8)}</td><td>{fmt(s.away_vga8)}</td></tr><tr><td>Capped actual GA (±1 xG)</td><td>{fmt(s.home_vga8_capped)}</td><td>{fmt(s.away_vga8_capped)}</td></tr><tr><td>Raw xGA</td><td>{fmt(s.home_vxga8)}</td><td>{fmt(s.away_vxga8)}</td></tr><tr><td><b>Adjusted xGA</b></td><td><b>{fmt(calc?.aha)}</b></td><td><b>{fmt(calc?.aaa)}</b></td></tr>
    </tbody></table></div>
   ...
   <div className="muted" style={{marginTop:8}}>Market: {f.market_source||"—"}{f.market_snapshot_at?` · snapshot ${dayTime(f.market_snapshot_at)}`:""}. Goal/xG residual cap: ±{fmt(s?.goal_xg_residual_cap,1)}.</div>
  </section>)}</div>
 </>
}
