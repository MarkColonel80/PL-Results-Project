...
export default function WeekendVenueReview(){
 ...
 return <>
  <div className="hero"><div><h1>Weekend venue review</h1><div className="muted">No prediction is made until both teams have at least 4 relevant venue matches. Once eligible, venue PPG8 decides unless the venue PPG gap is ≤ 0.30; only then does adjusted venue xG8 break the tie.</div></div><Link href="/betting">← Betting Lab</Link></div>
  <section className="card"><b>Adjusted venue xG rule</b><div className="muted" style={{marginTop:6}}>A hard minimum of <b>4 relevant venue matches per team</b> applies before the model produces expected goals, H/D/A probabilities, fair odds or a pick. Samples of 4–7 matches are eligible partial Venue8 samples; 8 is the full window. Actual goals are capped only when they exceed match xG by more than 1.0 goal: capped actual = min(actual, xG + 1.0), then blended 50/50 with raw venue xG. The separate <b>Diagnostics only</b> section is for context and does not alter the pick, expected goals, probabilities or fair odds. All prices below are decimal odds; bookmaker probabilities are no-vig estimates from the Oddschecker 1X2 prices.</div></section>
  <div style={{display:"grid",gap:14}}>{cards.map(({f,s,m,calc})=><section className="card" key={f.fixture_id}>
   ...
   {!s?<p>No snapshot.</p>:<>
    <div style={{marginTop:12}}><b>Calculation inputs</b><div className="muted" style={{marginTop:4}}>These figures feed the weekend calculation only when both teams meet the 4-match minimum.</div></div>
