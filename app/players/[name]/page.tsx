...
export default function Player(){
 ...
 useEffect(()=>{(async()=>{
  ...
  const [{data:g},{data:a},{data:pm},{data:fp},{data:history}]=await Promise.all([
   ...
   supabase.from("player_match_stats").select("match_id,gameweek,minutes_played,is_starting,goals,assists,xg,xa,shots,shots_on_target,chances_created,source").eq("season",season).eq("player_code",playerCode).gt("minutes_played",0).order("gameweek",{ascending:false}),
