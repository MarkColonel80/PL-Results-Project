...
def fetch(path,optional=False):
 try:
  req=urllib.request.Request(url(path),headers={"User-Agent":"pl-data-updater/3.0"})
...
for gw in range(1,39):
 ...
 player_meta={str(r.get("player_id")):{"name":r.get("web_name") or " ".join(x for x in [r.get("first_name"),r.get("second_name")] if x),"team_code":str(r.get("team_code") or "")} for r in gw_players if r.get("player_id")}
 for r in fetch(root+"/playermatchstats.csv",True):
  rr=dict(r);rr["__gw"]=gw;rr["__meta"]=player_meta.get(str(r.get("player_id")),{})
  all_pms.append(rr)
...
for r in all_pms:
 ...
 pms_by_key[(mid,pid)]={"season":SEASON_LABEL,"gameweek":I(all_matches[mid].get("gameweek"),r.get("__gw")),"match_id":mid,"player_id":pid,"player_name":meta.get("name") or pid,"team_name":team_by_code.get(team_code,team_code or None),"minutes_played":mins,"is_starting":bool(mins>0 and start_min==0),"goals":I(r.get("goals"),0) or 0,"assists":I(r.get("assists"),0) or 0,"xg":F(r.get("xg")),"xa":F(r.get("xa")),"shots":I(r.get("total_shots")),"shots_on_target":I(r.get("shots_on_target")),"chances_created":I(r.get("chances_created"))}
...
for r in all_incidents:
 ...
 goal_rows.append({"match_id":mid,"season":SEASON_LABEL,"gameweek":I(m.get("gameweek")),"incident_index":incident_index,"home_team":team_by_code.get(hc,hc),"away_team":team_by_code.get(ac,ac),"minute":I(r.get("minute")),"added_time":I(r.get("added_time")),"team_side":r.get("team_side"),"player_id":str(r.get("player_id")) if r.get("player_id") else None,"player_name":r.get("player_name"),"assist_player_id":str(r.get("assist_player_id")) if r.get("assist_player_id") else None,"assist_player_name":r.get("assist_player_name"),"goal_type":r.get("goal_type"),"home_score":I(r.get("home_score")),"away_score":I(r.get("away_score"))})
